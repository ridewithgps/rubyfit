...
static void pass_err_message(char *msg) {
...
static VALUE parser_init(VALUE self, VALUE handler) {
	cFitHandler = handler;
...
static VALUE parser_parse(VALUE self, VALUE original_str) {
	int i = 0;
...
void rf_parser_define(VALUE cFitParser) {
	//instance methods
	rb_define_method(cFitParser, "initialize", parser_init, 1);
	rb_define_method(cFitParser, "parse", parser_parse, 1);
