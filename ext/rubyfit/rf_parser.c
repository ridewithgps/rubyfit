...
static void handler_invoke(VALUE self, VALUE method, int argc,  VALUE param) {
    VALUE handler = rb_ivar_get(self, HANDLER_ATTR);
    rb_funcall(handler, method, argc, param);
}

static void pass_message(VALUE self, char *msg) {
    handler_invoke(self, cFitHandlerPrintFun, 1, rb_str_new2(msg));
}

static void pass_err_message(VALUE self, char *msg) {
    handler_invoke(self, cFitHandlerPrintErrFun, 1, rb_str_new2(msg));
}

static void pass_activity(VALUE self, const FIT_ACTIVITY_MESG *mesg) {
    ...
    handler_invoke(self, cFitHandlerActivityFun, 1, rh);
}

static void pass_record(VALUE self, const FIT_RECORD_MESG *mesg) {
    VALUE rh = rb_hash_new();
    ...
    if (mesg->left_right_balance != FIT_UINT8_INVALID)
        rb_hash_aset(rh, rb_str_new2("left_right_balance"),
                     UINT2NUM(mesg->left_right_balance & FIT_LEFT_RIGHT_BALANCE_MASK));
    ...
    handler_invoke(self, cFitHandlerRecordFun, 1, rh);
}

static void pass_lap(VALUE self, const FIT_LAP_MESG *mesg) {
    VALUE rh = rb_hash_new();
    ...
    handler_invoke(self, cFitHandlerLapFun, 1, rh);
}

static void pass_session(VALUE self, const FIT_SESSION_MESG *mesg) {
    VALUE rh = rb_hash_new();
    ...
    handler_invoke(self, cFitHandlerSessionFun, 1, rh);
}

static void pass_user_profile(VALUE self, const FIT_USER_PROFILE_MESG *mesg) {
    VALUE rh = rb_hash_new();
    ...
    handler_invoke(self, cFitHandlerUserProfileFun, 1, rh);
}

static void pass_event(VALUE self, const FIT_EVENT_MESG *mesg) {
    VALUE rh = rb_hash_new();
    ...
    handler_invoke(self, cFitHandlerEventFun, 1, rh);
}

static void pass_device_info(VALUE self, const FIT_DEVICE_INFO_MESG *mesg) {
    VALUE rh = rb_hash_new();
    ...
    handler_invoke(self, cFitHandlerDeviceInfoFun, 1, rh);
}

static void pass_weight_scale_info(VALUE self, const FIT_WEIGHT_SCALE_MESG *mesg) {
    VALUE rh = rb_hash_new();
    ...
    handler_invoke(self, cFitHandlerWeightScaleInfoFun, 1, rh);
}
                ...
                case FIT_CONVERT_MESSAGE_AVAILABLE: {
                    ...
                    switch (mesg_num) {
                        ...
                        case FIT_MESG_NUM_USER_PROFILE: {
                            ...
                            pass_user_profile(self, user_profile);
                            break;
                        }

                        case FIT_MESG_NUM_ACTIVITY: {
                            const FIT_ACTIVITY_MESG *activity = (FIT_ACTIVITY_MESG *) mesg;
                            pass_activity(self, activity);

                            {
                                ...
                                sprintf(err_msg,
                                        "Restored num_sessions=1 - Activity: timestamp=%u, type=%u, event=%u, event_type=%u, num_sessions=%u\n",
                                        activity->timestamp, activity->type, activity->event, activity->event_type,
                                        activity->num_sessions);
                                pass_message(self, err_msg);
                            }
                            break;
                        }
...
void rf_parser_define(VALUE cFitParser) {
    //instance methods
